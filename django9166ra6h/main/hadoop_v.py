...
def upload_csv_mapreduce_hadoop():
    # 查询数据
    query = "SELECT * FROM guangzhouxf"
    df = pd.read_sql(query, connection)
    local_csv_path = os.path.join(parent_directory,"guangzhouxf.csv")
    # 导出为 CSV 文件
    df.to_csv(local_csv_path, index=False)
    print(f"数据成功导出到 CSV 文件: {local_csv_path}")
    hdfs_csv_path = '/input/guangzhouxf.csv'
    # 目标 HDFS 路径
...
def send_cmd():
    ...
    [
        ...
        "-mapper", f"\"python value_mapper.py {csv_index('guangzhouxf.csv','title')} {csv_index('guangzhouxf.csv','avgprice')} 无  \"",
        "-reducer", "\"python value_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/guangzhouxf.csv",
        "-output", "hdfs://localhost:9000/output/guangzhouxf/valuetitleavgprice"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('guangzhouxf.csv','title')}\"",
        "-reducer", "\"python group_reducer.py title\"",
        "-input", "hdfs://localhost:9000/input/guangzhouxf.csv",
        "-output", "hdfs://localhost:9000/output/guangzhouxf/grouptitle"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('guangzhouxf.csv','jianmian')}\"",
        "-reducer", "\"python group_reducer.py jianmian\"",
        "-input", "hdfs://localhost:9000/input/guangzhouxf.csv",
        "-output", "hdfs://localhost:9000/output/guangzhouxf/groupjianmian"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('guangzhouxf.csv','housetype')}\"",
        "-reducer", "\"python group_reducer.py housetype\"",
        "-input", "hdfs://localhost:9000/input/guangzhouxf.csv",
        "-output", "hdfs://localhost:9000/output/guangzhouxf/grouphousetype"
    ],
    [
        f"{hadoop_path}/bin/hadoop.cmd", "jar", f"{hadoop_path}/share/hadoop/tools/lib/hadoop-streaming-3.3.0.jar",
        "-files", "\"hdfs://localhost:9000/input/group_mapper.py,hdfs://localhost:9000/input/group_reducer.py\"",
        "-mapper", f"\"python group_mapper.py {csv_index('guangzhouxf.csv','properright')}\"",
        "-reducer", "\"python group_reducer.py properright\"",
        "-input", "hdfs://localhost:9000/input/guangzhouxf.csv",
        "-output", "hdfs://localhost:9000/output/guangzhouxf/groupproperright"
    ],
    ]
